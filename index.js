      ...
      for (var i = 0; i < res.length; i++) {
        ...
        gitlabApi(`/projects/${namespace}/issues/${iid}`)
          .then(res => {
            ...
            let issueMsg = `
><${env.GITLAB_URL}/${parts[0]}/issues/${iid}|${parts[0]}#${parts[1]}>: \`${
  res.state
}\` ${res.title}
>${assignee} | ${time}
                    `
    ...
    function (bot, message) {
      ...
      matcher('#', message).then(res => {
        for (var i = 0; i < res.length; i++) {
          ...
          gitlabApi(`/projects/${namespace}/issues/${iid}`)
            .then(res => {
              let time = moment(res.created_at).format(
    ...
    function (bot, message) {
      ...
      matcher('%', message).then(res => {
        for (var i = 0; i < res.length; i++) {
          ...
          let namespace = encodeURIComponent(text.namespace)
          let iid = text.iid
          gitlabApi(`/projects/${namespace}/milestones?iids=${iid}`)
            .then(response => {
              ...
              if (response.message || response.length === 0) {
                ...
                let issueMsg = `
<${env.GITLAB_URL}/${parts[0]}/milestones/${iid}|${parts[0]}%${parts[1]}> \`${
  res.state
}\` ${res.title}
>*Description*
>${description}
